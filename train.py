...
def train(model, training_data, validation_data, optimizer, device, opt):
    ...
    for epoch_i in range(opt.epoch):
        ...
        checkpoint = {'epoch': epoch_i, 'settings': opt, 'model': model.state_dict(),
                      'src_vocab': training_data.dataset.fields['src'].vocab,
                      'trg_vocab': training_data.dataset.fields['trg'].vocab}
...
def main():
    ...
    transformer = Transformer(
        ...
        d_word_vec=opt.d_word_vec, h=opt.n_head, d_model=opt.d_model, d_k=opt.d_k, d_q=64, N=opt.n_layers
    ).to(device)
